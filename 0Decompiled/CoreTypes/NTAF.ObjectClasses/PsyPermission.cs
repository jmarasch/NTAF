...
{
	[ObjectClassPlugIn("PsyPermission", "0.0.0.0")]
	[Serializable]
	public class PsyPermission : WSPPermission
	{
		[Browsable(false)]
		[XmlIgnore]
		...
		public PsyPermission(string pID, string pName, Species MyBaseRace)
		{
			base.ID = pID;
			base.Name = pName;
		...
		public PsyPermission(string pID, string pName, Race MyRace)
		{
			base.ID = pID;
			base.Name = pName;
		...
		public PsyPermission(PsyPermission permission)
		{
		...
		public PsyPermission(string pID, string pName, Species MyBaseRace, Race MyRace)
		{
			base.ID = pID;
			base.Name = pName;
			this.SpeciesCanEquip = MyBaseRace;
		...
		{
			bool d = true;
			if ((obj == null ? true : !(obj is PsyPermission)))
			{
				d = false;
			}
			if (d)
			{
				d = base.ID == ((PsyPermission)obj).ID;
			}
			if (d)
			{
				d = base.Name == ((PsyPermission)obj).Name;
			}
			if (d)
			{
				d = this.RaceCanEquip == ((PsyPermission)obj).RaceCanEquip;
			}
			if (d)
			{
				d = this.SpeciesCanEquip == ((PsyPermission)obj).SpeciesCanEquip;
			}
			return d;
		}
		...
		{
			return new DataMember[] { new DataMember("Name", base.Name), new DataMember("Type", "Psy Permission"), new DataMember("Race Can Equip", this.RaceCanEquip), new DataMember("Species Can Equip", (object)this.SpeciesCanEquip) };
		}
		...
		public static bool operator ==(PsyPermission a, PsyPermission b)
		{
			bool flag = false;
			try
			{
		...
		{
			element.AddTitleText(string.Concat("Psy Permission: ", base.Name));
			if (this.RaceCanEquip == null)
			{
		...
		{
			return new PsyPermission.sortIDHelper_ASC();
		}

		public static IComparer sortIDDsc()
		{
			return new PsyPermission.sortIDHelper_DSC();
		}

		public static IComparer sortNameDsc()
		{
			return new PsyPermission.sortNameHelpe_DSC();
		}
